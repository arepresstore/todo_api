...
class UserViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [permissions.AllowAny]  # Разрешаем регистрацию без аутентификации


# Кастомный аутентификатор для возврата токена
...
class TaskViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        # Возвращаем задачи, где пользователь владелец или имеет права доступа
    ...
    def perform_create(self, serializer):
        # Автоматически устанавливаем владельца при создании задачи
        serializer.save(owner=self.request.user)

    @action(detail=True, methods=['get'])
    def permissions(self, request, pk=None):
        # Получаем список прав доступа для задачи
...
# Представление для прав доступа
class TaskPermissionViewSet(viewsets.ModelViewSet):
    serializer_class = TaskPermissionSerializer
    permission_classes = [permissions.IsAuthenticated]

    def get_queryset(self):
        # Возвращаем только те права, где текущий пользователь владелец задачи
